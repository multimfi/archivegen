...
func mapAccess(f string) error {
	if _, exists := qemu[f]; !exists {
		return errorNotFound{f}
	}
	return nil
}

func mapOpen(f string) (elfFile, error) {
	r, exists := qemu[f]
	if !exists {
...
func TestResolve(t *testing.T) {
	...
	r, err := Resolve("/usr/bin/qemu-system-x86_64")
	if err != nil {
		t.Fatal(err)
	}

	if w, r := len(qR), len(r); w != r {
